...
- **leghe.fantacalcio.it (login automatico)**: valutato per calendario/avversari/formazioni della lega privata dell'utente. Il login (`PUT utente/login`) risponde in base64 e il client ufficiale allega un header `app_key` segreto per autenticare l'app stessa, non solo l'utente: è un'API privata non documentata, diversa dalle pagine pubbliche già usate per quotazioni/voti. Estrarre quella chiave per impersonare il client ufficiale è stato scartato (non per i termini d'uso, esplicitamente non un problema per l'utente in questo caso, ma per il tipo di accesso non autorizzato che rappresenterebbe). Sostituito con import manuale, vedi `CalendarioGiornata` nello schema Prisma e la schermata "Calendario" nell'app: l'utente copia a mano giornata/avversario dal sito.
